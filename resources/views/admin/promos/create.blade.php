<x-layouts.admin title="Tambah Promo">
    @if ($errors->any())
    ...
    <div class="container mx-auto p-10">
        <div class="card bg-base-100 shadow-sm">
            <div class="card-body">
                <h2 class="card-title text-2xl mb-6">Tambah Promo</h2>
                ...
                <form action="{{ route('admin.promos.store') }}" method="POST" class="space-y-4">
                    ...
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="nama" value="{{ old('nama') }}" placeholder="Contoh: Promo Lebaran"
                    ...
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="kode" value="{{ old('kode') }}" placeholder="Contoh: LEBARAN10"
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="form-control w-full">
                            ...
                            <select name="tipe" class="select select-bordered @error('tipe') select-error @enderror" required>
                                <option value="percent" @selected(old('tipe') === 'percent')>Persen</option>
                                <option value="fixed" @selected(old('tipe') === 'fixed')>Nominal</option>
                            </select>
                        </div>
                        <div class="form-control w-full">
                            <label class="label"><span class="label-text font-semibold">Nilai</span></label>
                            <input type="number" step="0.01" name="nilai" value="{{ old('nilai') }}" placeholder="Contoh: 10 atau 50000"
                    ...
                    <div class="form-control w-full">
                        ...
                        <select id="scopeSelect" name="scope" class="select select-bordered @error('scope') select-error @enderror" required>
                            <option value="all" @selected(old('scope') === 'all')>Semua</option>
                            <option value="event" @selected(old('scope') === 'event')>Event</option>
                            <option value="ticket_type" @selected(old('scope') === 'ticket_type')>Tipe Tiket</option>
                            <option value="ticket" @selected(old('scope') === 'ticket')>Tiket Spesifik</option>
                        </select>
                    </div>
                    ...
                    <div id="eventField" class="form-control w-full hidden">
                        ...
                        <select name="event_id" class="select select-bordered @error('event_id') select-error @enderror">
                            ...
                            @foreach($events as $event)
                                <option value="{{ $event->id }}" @selected(old('event_id') == $event->id)>{{ $event->judul }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div id="ticketTypeField" class="form-control w-full hidden">
                        ...
                        <select name="ticket_type_id" class="select select-bordered @error('ticket_type_id') select-error @enderror">
                            ...
                            @foreach($ticketTypes as $ticketType)
                                <option value="{{ $ticketType->id }}" @selected(old('ticket_type_id') == $ticketType->id)>{{ $ticketType->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div id="ticketField" class="form-control w-full hidden">
                        ...
                        <select name="tiket_id" class="select select-bordered @error('tiket_id') select-error @enderror">
                            ...
                            @foreach($tikets as $tiket)
                                <option value="{{ $tiket->id }}" @selected(old('tiket_id') == $tiket->id)>
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="form-control w-full">
                            ...
                            <input type="datetime-local" name="mulai" value="{{ old('mulai') }}" class="input input-bordered w-full @error('mulai') input-error @enderror" />
                        </div>
                        <div class="form-control w-full">
                            <label class="label"><span class="label-text font-semibold">Akhir</span></label>
                            <input type="datetime-local" name="akhir" value="{{ old('akhir') }}" class="input input-bordered w-full @error('akhir') input-error @enderror" />
                        </div>
                    </div>
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="form-control w-full">
                            ...
                            <input type="number" step="0.01" name="min_transaksi" value="{{ old('min_transaksi') }}" placeholder="Contoh: 100000"
                        ...
                        <div class="form-control w-full">
                            ...
                            <input type="number" name="kuota" value="{{ old('kuota') }}" placeholder="Contoh: 100"
