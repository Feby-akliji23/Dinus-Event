<div class="drawer-side is-drawer-close:overflow-visible ">
    ...
    <div class="flex min-h-full flex-col items-start bg-white border-r border-slate-200 w-64 is-drawer-close:w-14 is-drawer-open:w-80">
        <div class="w-full flex items-center justify-center p-4 border-b border-slate-100">
            <img src="{{ asset('assets/images/logo_bengkod.png') }}" alt="Logo" class="h-20 w-20 object-contain">
        </div>

        <!-- Sidebar content here -->
        <ul class="menu w-full grow gap-1 px-2 py-4 text-sm text-slate-600">
            <!-- Dashboard Item -->
            <li class="{{ request()->routeIs('admin.dashboard') ? 'bg-slate-100 text-slate-900 font-semibold rounded-lg' : '' }}">
                <a href="{{ route('admin.dashboard') }}" class="rounded-lg gap-3 hover:bg-slate-100 is-drawer-close:tooltip is-drawer-close:tooltip-right" data-tip="Dashboard">
                    <!-- Home icon -->
...
<!-- Kategori item -->
<li class="{{ request()->routeIs('admin.categories.*') ? 'bg-slate-100 text-slate-900 font-semibold rounded-lg' : '' }}">
	<a href="{{ route('admin.categories.index') }}" class="rounded-lg gap-3 hover:bg-slate-100 is-drawer-close:tooltip is-drawer-close:tooltip-right" data-tip="Kategori">
		<!-- icon Kategori -->
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
...
<!-- History item -->
<li class="{{ request()->routeIs('admin.histories.*') ? 'bg-slate-100 text-slate-900 font-semibold rounded-lg' : '' }}">
	<a href="{{ route('admin.histories.index') }}" class="rounded-lg gap-3 hover:bg-slate-100 is-drawer-close:tooltip is-drawer-close:tooltip-right" data-tip="History">
        ...
        <!-- logout -->
        <div class="w-full p-4 border-t border-slate-100">
